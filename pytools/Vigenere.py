...
class Vigenere(PolyalphaCipher):
    ...
    def __guess_key(self, min_len=1, max_len=9, display=False):
        ...
        for i in range(keylen):
            sub_alphabet = Vigenere(''.join([self._s[keylen*j + i] for j in range(self._len//keylen)]))
            min_differential = 99999
            password_letter = ""
            for c in range(65, 65+26):
                sub_alphabet.key = chr(c)
                decrypted = Vigenere(sub_alphabet.decipher().get_string())
